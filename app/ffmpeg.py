...
class FFmpegRunner(ABC):
    ...
    def _build_ffmpeg_args(self) -> list[str]:
        ...
        return [
            ...
            "-stimeout", str(self.config.rtsp_timeout),
            "-rw_timeout", str(self.config.rw_timeout),
            "-i", self.config.rtsp_url,
            "-vn",  # No video
            "-ac", str(self.config.channels),  # Audio channels
...
class FakeFFmpegRunner(FFmpegRunner):
    ...
    def __init__(self, config: FFmpegConfig, audio_data: Optional[bytes] = None):
    ...
    def set_failure_mode(self, should_fail: bool = True, fail_after_chunks: int = 0) -> None:
    ...
    async def read_audio_data(self) -> AsyncGenerator[bytes, None]:
        ...
        while self.is_running:
            ...
            if self._fail_after_chunks > 0 and self._chunks_yielded >= self._fail_after_chunks:
                self.is_running = False
...
def create_ffmpeg_runner(config: FFmpegConfig, fake: bool = False, audio_data: Optional[bytes] = None) -> FFmpegRunner:
