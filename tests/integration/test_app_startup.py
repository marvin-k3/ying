...
class TestAppStartup:
    ...
    def test_app_startup_lifecycle(self, mock_migration_manager_class, mock_worker_manager_class):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                with TestClient(app) as client:
                    ...
                    assert response.json() == {"status": "healthy", "service": "rtsp-music-tagger"}
    ...
    @patch('app.main.WorkerManager')
    @patch('app.main.MigrationManager')
    def test_worker_manager_error_handling(self, mock_migration_manager_class, mock_worker_manager_class):
...
class TestMethodNameValidation:
    ...
    def test_migration_manager_has_correct_methods(self):
        ...
        assert hasattr(manager, 'migrate_all'), "MigrationManager should have migrate_all method"
        assert callable(getattr(manager, 'migrate_all')), "migrate_all should be callable"
        
        # Check that old method names don't exist
        assert not hasattr(manager, 'apply_migrations'), "Old method name apply_migrations should not exist"
    ...
    def test_worker_manager_has_correct_methods(self):
        ...
        # Check that required methods exist
        assert hasattr(manager, 'start_all'), "WorkerManager should have start_all method"
        assert callable(getattr(manager, 'start_all')), "start_all should be callable"
        
        assert hasattr(manager, 'stop_all'), "WorkerManager should have stop_all method"
        assert callable(getattr(manager, 'stop_all')), "stop_all should be callable"
