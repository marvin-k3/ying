...
from app.ffmpeg import FFmpegConfig, FakeFFmpegRunner
...
class FakeMusicRecognizer(MusicRecognizer):
    ...
    def __init__(self, provider: str, results: List[Optional[RecognitionResult]] = None) -> None:
        self.provider = provider
        self.results = results or []
        self.call_count = 0
        self.last_wav_bytes: Optional[bytes] = None
        self.last_timeout: Optional[float] = None
    
    async def recognize(self, wav_bytes: bytes, timeout_seconds: float = 30.0) -> RecognitionResult:
        self.call_count += 1
...
class TestParallelRecognizers:
    ...
    async def test_capacity_limits(self):
        ...
        async def slow_recognize(wav_bytes: bytes, timeout_seconds: float) -> RecognitionResult:
...
class TestStreamWorker:
    ...
    async def test_worker_lifecycle(self, config, clock, repos):
        ...
        recognizers = {
            "shazam": FakeMusicRecognizer("shazam")
        }
...
class TestBackpressureAndCapacity:
    ...
    async def test_mixed_provider_fairness(self):
        ...
        async def shazam_recognize(wav_bytes: bytes, timeout_seconds: float) -> RecognitionResult:
        ...
        async def acoustid_recognize(wav_bytes: bytes, timeout_seconds: float) -> RecognitionResult:
