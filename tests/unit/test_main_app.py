...
class TestCreateApp:
    ...
    def test_create_app_includes_required_routes(self):
        ...
        # Basic endpoints
        assert "/healthz" in [route.path for route in app.routes if hasattr(route, 'path')]
        assert "/metrics" in [route.path for route in app.routes if hasattr(route, 'path')]
    ...
    def test_create_app_includes_middleware(self):
        ...
        middleware_types = [getattr(middleware.cls, '__name__', str(middleware.cls)) for middleware in app.user_middleware]
...
class TestLifespan:
    ...
    async def test_lifespan_migration_method_name(
        ...
        assert not hasattr(mock_migration_manager, 'apply_migrations') or \
               not mock_migration_manager.apply_migrations.called
    ...
    async def test_lifespan_worker_method_names(
        ...
        assert not hasattr(mock_worker_manager, 'start') or \
               not mock_worker_manager.start.called
        assert not hasattr(mock_worker_manager, 'stop') or \
               not mock_worker_manager.stop.called
