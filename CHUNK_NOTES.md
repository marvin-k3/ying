...
## Raw PCM Format Implementation for Symphonia Compatibility (Latest)
...
**Root Cause**: The WAV format being generated by FFmpeg was causing Symphonia (used by shazamio) to encounter malformed headers and emit warnings about "junk bytes" and "invalid mpeg audio header".

**Solution**: Switched to raw PCM format to eliminate format parsing issues entirely:

1. **Raw PCM Output from FFmpeg**:
   - Changed from `-f wav` to `-f s16le` (16-bit signed little-endian PCM)
   - Added explicit `-acodec pcm_s16le` parameter
   - Added `-sample_fmt s16` for explicit sample format specification
   - Added `-avoid_negative_ts make_zero` to avoid timestamp issues
   - Added `-fflags +genpts` to generate presentation timestamps
   - This eliminates all WAV header complexity and sends pure audio data

2. **Simplified Audio Processing**:
   - Removed WAV header validation and cleaning functions
   - Removed complex WAV format parsing
   - Now sends raw PCM data directly to Shazam
   - Shazam handles resampling and mono conversion internally

3. **Improved Error Handling**:
   - Simple PCM data length validation (minimum 1024 bytes)
   - Clear error messages for debugging
   - No more format-specific parsing issues

**Files Modified**:
- `app/ffmpeg.py`: Changed from WAV to raw PCM format (`-f s16le`)
- `app/recognizers/shazamio_recognizer.py`: Removed WAV validation/cleaning, simplified to handle raw PCM data
- `tests/unit/test_ffmpeg.py`: Updated expected FFmpeg arguments for PCM format
- `tests/unit/test_recognizers_shazamio.py`: Updated tests to use PCM data instead of WAV data

**Benefits**:
- **Complete Elimination of Symphonia Warnings**: No more "invalid mpeg audio header" or "skipping junk bytes" warnings
- **Simplified Processing**: No complex WAV header parsing or validation
- **Better Performance**: Direct PCM data transfer without format conversion overhead
- **More Reliable**: Raw audio data eliminates format-specific parsing issues
- **Shazam-Native**: Lets Shazam handle audio processing internally as designed

**Technical Details**:
- **Format**: 16-bit signed little-endian PCM (`s16le`)
- **Sample Rate**: 44.1 kHz (configurable)
- **Channels**: Mono (configurable)
- **Data Flow**: FFmpeg → Raw PCM → Shazam (with internal resampling)

**Testing Results**: All tests pass (238 passed, 5 skipped) with 90.39% coverage.
