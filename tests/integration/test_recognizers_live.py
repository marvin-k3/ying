...
class TestAcoustIDIntegration:
    ...
    @pytest.mark.asyncio
    async def test_acoustid_fingerprint_generation(
        self, sample_audio_data, acoustid_api_key
    ):
        """Test fingerprint generation with real audio."""
        recognizer = AcoustIDRecognizer(
            api_key=acoustid_api_key, chromaprint_path="/usr/bin/fpcalc"
        )

        try:
            # Test fingerprint generation directly
            fingerprint = await recognizer._generate_fingerprint(
                sample_audio_data, 30.0
            )
            ...
            if fingerprint:
                ...
                pytest.skip(
                    "Could not generate fingerprint - fpcalc may not be available"
                )
    ...
    async def test_acoustid_invalid_api_key(self, sample_audio_data):
        ...
        recognizer = AcoustIDRecognizer(
            api_key="invalid_key_12345", chromaprint_path="/usr/bin/fpcalc"
        )
...
class TestParallelIntegration:
    ...
    async def test_parallel_shazam_acoustid_real_apis(self, sample_audio_data):
        ...
        try:
            ...
            if acoustid:
                tasks.append(
                    acoustid.recognize(sample_audio_data, timeout_seconds=30.0)
                )
            ...
            for i, result in enumerate(results):
                ...
                assert not isinstance(result, Exception), (
                    f"{provider} should not raise exceptions"
                )
...
async def test_integration_environment_check():
    ...
    print(
        f"   fpcalc binary: {'✅ Found at ' + fpcalc_path if fpcalc_path else '❌ Not found'}"
    )
