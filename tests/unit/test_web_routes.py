...
class TestUtilityFunctions:
    ...
    def test_convert_utc_to_pt(self) -> None:
        ...
        utc_tz = pytz.UTC
        utc_dt_aware = utc_tz.localize(datetime(2024, 7, 15, 20, 30, 45))  # Summer (PDT)
...
class TestWebRoutes:
    ...
    def app_config(self) -> Config:
        ...
        config.stream_1 = StreamConfig(name="living_room", url="rtsp://test1", enabled=True)
    ...
    async def test_get_plays_json_with_stream_filter(
        ...
        # Mock repository
        mock_repo = AsyncMock()
        mock_repo.get_plays_by_date.return_value = [sample_plays_data[0]]  # Only first play
        mock_repo_class.return_value = mock_repo
    ...
    async def test_get_plays_csv_format(
        ...
        # Check header
        assert rows[0] == [
            "Time (PT)", "Title", "Artist", "Album", "Stream", 
            "Confidence", "Track ID", "UTC Timestamp"
        ]
...
class TestCSVGeneration:
    ...
    def test_csv_filename_generation(self) -> None:
        ...
        from app.web.routes import generate_csv_response, PlayRecord
        ...
        assert "plays_2024-01-15_living_room.csv" in response.headers["content-disposition"]
