...
from app.recognizers.acoustid_recognizer import AcoustIDRecognizer, FakeAcoustIDRecognizer
...
class TestAcoustIDRecognizer:
    ...
    async def test_successful_recognition(self, acoustid_fixtures):
        ...
        with patch.object(recognizer, "_generate_fingerprint", return_value="fake_fingerprint"), \
             patch.object(recognizer, "_query_acoustid", return_value=acoustid_fixtures["successful_match"]):
    ...
    async def test_high_confidence_recognition(self, acoustid_fixtures):
        ...
        with patch.object(recognizer, "_generate_fingerprint", return_value="fake_fingerprint"), \
             patch.object(recognizer, "_query_acoustid", return_value=acoustid_fixtures["high_confidence_match"]):
    ...
    async def test_low_confidence_recognition(self, acoustid_fixtures):
        ...
        with patch.object(recognizer, "_generate_fingerprint", return_value="fake_fingerprint"), \
             patch.object(recognizer, "_query_acoustid", return_value=acoustid_fixtures["low_confidence_match"]):
    ...
    async def test_no_match_found(self, acoustid_fixtures):
        ...
        with patch.object(recognizer, "_generate_fingerprint", return_value="fake_fingerprint"), \
             patch.object(recognizer, "_query_acoustid", return_value=acoustid_fixtures["no_match"]):
    ...
    async def test_api_error_response(self, acoustid_fixtures):
        ...
        with patch.object(recognizer, "_generate_fingerprint", return_value="fake_fingerprint"), \
             patch.object(recognizer, "_query_acoustid", return_value=acoustid_fixtures["error_response"]):
    ...
    async def test_http_error_response(self):
        ...
        # Setup
        recognizer = AcoustIDRecognizer(api_key="test_key")
        http_error_response = {"error": {"message": "AcoustID API error: HTTP 400", "code": 400}}
        
        with patch.object(recognizer, "_generate_fingerprint", return_value="fake_fingerprint"), \
             patch.object(recognizer, "_query_acoustid", return_value=http_error_response):
    ...
    async def test_fingerprint_generation_success(self):
        ...
        # Setup
        recognizer = AcoustIDRecognizer(api_key="test_key", chromaprint_path="/fake/fpcalc")
    ...
    async def test_timeout_handling(self):
        ...
        with patch.object(recognizer, "_generate_fingerprint", side_effect=Exception("Timeout")):
            # Execute
...
class TestFakeAcoustIDRecognizer:
    ...
    async def test_successful_fixture_response(self, acoustid_fixtures):
        ...
        recognizer = FakeAcoustIDRecognizer(
            fixture_responses=acoustid_fixtures,
...
async def test_performance_multiple_parallel_calls():
    ...
    import asyncio
    tasks = [
        recognizer.recognize(b"fake_wav_data")
        for recognizer in recognizers
    ]
